...
void symtab_addParamInfo() {
    ...
    while(source->dtype != Function_type && source->dtype != Procedure_type) {
        Symtab *new = malloc(sizeof(Symtab));
        strcpy(new->name, source->name);    // name
        int type = new->dtype = source->dtype; // dtype
        new->next = NULL;
        new->arrNode = NULL;
        new->paraminfo = NULL;
        ...
        if(type == Array_type) {
            new->arraydtype = source->arraydtype; // arraydtype
            Array* source_arr = source->arrNode;

            while(source_arr){   // arrNode
                symtab_addArrayDim(new, source_arr->low, source_arr->high, false); // without allocating mem
